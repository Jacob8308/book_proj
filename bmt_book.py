...
flag = 0  # 一开始时预约肯定未开始
#######################*可修改参数begin*#######################
script_mode = 'it'  # 预约脚本的模式，'cd' for count down（等待下周一八点预约），'it' for instant（立即开始预约）
#######################*可修改参数end*#######################
# 主函数
...
def main():
    ...
    fellow_id = '2032417'  # 同行者学号
    selected_weekdays = [i for i in range(1, 8)]  # 选择预约的日期，数字代表星期几
    # 输入想预约场地的开始时间（请保持格式相同，list中元素数量可改）
    selected_start_time = ['18:00', '19:00', '20:00']
    # 自己登录预约网站任意请求头中的Authorization键值（一次登录，之后无需变动）
    Authorization_code = '51721a416b1ad808a82e59db8bfe5e7b'

    # *以下输入参数非必要无需更改！！！
    # 初始化各个场地项目编号
    campus_id = '1'  # 代表四平校区
    stadium_item_id = '1'  # 代表羽毛球项目
    # stadium_item_id = '3'  # 代表网球项目
    stadium_id = {'129': '1'}  # 代表项目的场馆编号，用dict储存
    stadium_location_id = {'129': ['12', '13', '24', '25']}  # 分别代表129的1~4号场，攀岩馆的1~4号场（5号场被嫌弃了。。。）
    #######################*可修改参数end*#######################
    ...
    for sta_id in stadium_id:
        for date in ava_date:
            for sta_loca_id in stadium_location_id[sta_id]:
                ...
                selected_start_time_copy = selected_start_time.copy()
                for each in site_list:
                    if (each['time_start'] in selected_start_time_copy):
                        if each['is_check'] == 1:
                            ...
                            while order_count <= 3:
                                ...
                                if result_status == 200:
                                    ...
                                    if result == 'ok':
                                        count = count+1
                                        selected_start_time_copy.remove(each['time_start'])
                                        print('\n成功预约！\n次数=%s\n' % count)
...
if script_mode == 'cd':
    ...
    while flag == 0:
        ...
        if de_sec >= 10 and de_sec < 20:  # 最后20s倒计时时开始main函数倒计时，保证倒计时的准确性
            tictoc = Timer(de_sec+1, main, ())  # 多一秒确保系统开启预约再运行主函数
            tictoc.start()
            print('开始预约倒计时~')
        if hours>1:
