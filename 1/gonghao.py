...
def main():
    ...
    fellow_id = '2032416'  # 同行者学号
    selected_weekdays = [1, 2, 3, 4, 5]  # 选择预约的日期，数字代表星期几
    # 输入想预约场地的开始时间（请保持格式相同，list中元素数量可改）
    selected_start_time = ['18:00', '19:00', '20:00']
    # 自己登录预约网站任意请求头中的Authorization键值（一次登录，之后无需变动）
    Authorization_code = 'bae0f3d9cbd63d46a7a5cc4569e645e9'

    # *以下输入参数非必要无需更改！！！
    # 初始化各个场地项目编号
    campus_id = '1'  # 代表四平校区
    stadium_item_id = '1'  # 1代表羽毛球项目 2代表乒乓球 3代表网球
    stadium_id = {'panyanguan': '2'}  # 代表项目的场馆编号，用dict储存
    stadium_location_id = {'panyanguan': ['31', '32', '33', '34', '35']}  # 分别代表129的1~4号场，攀岩馆的1~4号场（5号场被嫌弃了。。。）
    #######################*可修改参数end*#######################
...
if script_mode == 'cd':
    ...
    while flag == 0:
        ...
        if de_sec >= 10 and de_sec < 20:  # 最后20s倒计时时开始main函数倒计时，保证倒计时的准确性
            tictoc = Timer(de_sec+1, main, ())  # 多一定时间确保系统开启预约再运行主函数
            tictoc.start()
            print('开始预约倒计时~')
        if hours>1:
